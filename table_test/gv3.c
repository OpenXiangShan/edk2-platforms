// gen_einj_full.c
// Final version: based on gv2_fin-ok.c (working) + kernel-compliant EINJ actions
#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include <stdlib.h>
...
// === MEMORY LAYOUT (from gv2_fin-ok.c) ===
...
int main(void) {
    // === Step 1: Build ACPI EINJ Table (FIXED: only ONE action=2) ===
    struct acpi_whea_header einj_entries[] = {
        ...
        // SET_ERROR_TYPE — ONLY ONE ENTRY! (kernel writes error_type here)
        ...
        // GET_COMMAND_STATUS → write to ERROR_STATUS_ADDR (as in gv2_fin-ok.c)
    ...
    // --- Link ERROR_STATUS to ERROR_RECORD (critical!) ---
    *(uint64_t*)(workspace + ERROR_STATUS_OFFSET) = ERROR_RECORD_ADDR;
    ...
    // --- Trigger Entries (action=0xFF, last value = ERROR_RECORD_ADDR) ---
    ...
    uint64_t values[4] = {1, 2, 3, ERROR_RECORD_ADDR}; // ← Last is error record address!

    for (int i = 0; i < 4; i++) {
        trigger_entries[i] = (struct acpi_einj_trigger_entry){
            .action = 0xFF, // ← Must be 0xFF for trigger
            .instruction = 0x03, // WRITE_REGISTER_VALUE
            .flags = 0,
            .reserved = 0,
            .register_region = {
                .space_id = 0,
    ...
    error_record->error_severity = 2;        // Corrected
