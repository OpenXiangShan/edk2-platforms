  ...
  )
{
  ...
  for (Index = 0; Index < CONFIG_RX_DESCR_NUM; Index++) {
    ...
    RxDescriptor->Tdes1 = RDES1_CHAINED | (RX_MAX_PACKET & DESC_RXCTRL_SIZE1MASK);
  }
  ...
  )
{
  ...
  //IrqStat parameter needs to be checked, 
  //Be careful, IrqStat may be a null pointer    
  if (IrqStat == NULL) {
    Status = EFI_INVALID_PARAMETER;
    //DEBUG ((DEBUG_ERROR, "%a()  IrqStat is Null Status:%r\n", __FUNCTION__, Status));
  }
