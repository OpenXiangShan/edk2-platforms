...
//  EFI_STATUS Status;
  ...
  DEBUG((DEBUG_MMCHOST_SD, "TargetFrequency:%d\n", TargetFrequency));

  BaseFrequency = 25000000;
  ...
  )
{
  ...
  default:
    Print (L"MMCHost: MMCNotifyState(): Invalid State: %d\n", State);
    ASSERT (0);
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_MMCHOST_SD, "BoscSdhciHost: MMCInitialize()\n"));
